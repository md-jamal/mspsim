...
PACKAGES := se/sics/mspsim ${addprefix se/sics/mspsim/,core chip cli config debug platform ${addprefix platform/,esb sky smote jcreate sentillausb z1 tyndall ti wismote}plugin profiler emulink net ui util extutil/highlight extutil/jfreechart}
...
.PHONY: all compile jar help run runesb runsky runsmote test cputest $(CPUTEST) mtest
...
%.smote:	jar
	java -jar $(JARFILE) -platform=smote  $@ $(ARGS)
...
runsmote:	compile
	...
	$(JAVA) $(JAVAARGS) se.sics.mspsim.platform.smote.SmoteNode $(SMOTEFIRMWARE) $(MAPARGS) $(ARGS)
